...
class SWEEnv(gym.Env):
    ...
    def reset(self, index: int = None, apply_test_patch: bool = False) -> Tuple[str, dict]:
        ...
        * Check out base commit
        ...
        self.ref_patch = self.record['patch']
        #self.patch_replay = self.record['patch_replay']
        self.reward = None
        ...
        if repo_name not in folders:
            ...
            if not self.args.no_mirror and not self.is_from_github_url:
                ...
                result = subprocess.run(["docker", "ps", "-aqf", "name=sweagent-replay-swe-agent-latest"], capture_output=True, text=True)
                if result.returncode == 0:
                    container_id = result.stdout.strip()
                    command = ["docker", "cp", f"/home/zjw/SWE-Bench/repo/{repo_name}.tar.gz", f"{container_id}:/tmp/"]
                ...
                self.communicate_with_handling(
                    input=f"tar -xf /tmp/{repo_name}.tar.gz -C / && git config --global --add safe.directory /{repo_name} && rm /tmp/{repo_name}.tar.gz",
                ...
                result= subprocess.run(["docker", "ps", "-aqf", "name=swe-agent-latest"], capture_output=True, text=True)
                print(result.stdout)
                result = subprocess.run(["docker", "ps", "-aqf", "name=swe-agent-latest"], capture_output=True, text=True)
                if result.returncode == 0:
                    container_id = result.stdout.strip()
                ...
                self.communicate_with_handling(
                    input=f"tar -xf ./tmp/{repo_name}.tar.gz -C /{repo_name} && git config --global --add safe.directory /{repo_name} && rm ./tmp/{repo_name}.tar.gz",
                    error_msg="Failed to clone repository from mirror",
                    timeout_duration=LONG_TIMEOUT,
                )
...
ssl_verify: false
        '''
        self.communicate(f"echo '{new_channels.strip()}' > ~/.condarc")
        #print("---------------")
        #output = self.communicate("cat ~/.condarc")
        #print(output)
        #print("------------------")
    ...
    def _init_container(self) -> None:
        ...
        if self.container_name is None:
            ...
            image_name_sanitized = self.image_name.replace("/", "-replay-")
    ...
    '''
    def _communicate(self, input: str, timeout_duration=25) -> str:
    # Private method to handle actual communication with the container
        command = input if input.endswith("\n") else input + "\n"
        try:
            self.returncode = None
            ...
            for stdout_line in iter(process.stdout.readline, ""):
                print(stdout_line, end='')  # Real-time output to console
                stdout_buffer.append(stdout_line)
            
            process.stdout.close()
            stderr_output = process.communicate()[1]
            if stderr_output:
                print(stderr_output, file=sys.stderr)  # Real-time error output to console
                stderr_buffer.append(stderr_output)
            
            self.returncode = process.returncode

            if self.returncode != 0:
                raise RuntimeError(f"Command '{input}' failed with error:\n{stderr_output}")
            ...
            process.kill()
            self.returncode = 1
            raise RuntimeError(f"Command '{input}' timed out after {timeout_duration} seconds")
    '''
    ...
    def install_env(self) -> None:
        ...
        if env_check.strip() == "":
            ...
            #self.communicate_with_handling("conda update conda", error_msg="Fail to update conda!")
            #print("000")
            #self.communicate_with_handling("conda config —remove-key channels", error_msg="Failed to remove other channel!")
            #self.communicate_with_handling("conda config --add channels https://mirrors.tuna.tsinghua.edu.cn/anaconda/pkgs/free/", error_msg="Fail to setting tsinghua image!")
            #self.communicate_with_handling("conda config --add channels https://mirrors.tuna.tsinghua.edu.cn/anaconda/pkgs/main/", error_msg="Fail to setting tsinghua image!")
            #self.communicate_with_handling("conda config --set show_channel_urls yes", error_msg="Fail to setting tsinghua image!")
            #self.communicate_with_handling("ping baidu.com", error_msg="error!")
            #print("111")
            ...
            if "pip_packages" in install_configs:
                print(f"pip install {install_configs['pip_packages']}")
                self.communicate_with_handling(
                    f"source activate {env_name}",
                    error_msg="activate failed",
                    timeout_duration=LONG_TIMEOUT
                )
                # 使用列表中的元素构建一个空格分隔的字符串
                pip_packages_str = ' '.join(install_configs['pip_packages'])
                # 执行 pip install 命令，并传递正确格式的要求字符串
                self.communicate_with_handling(

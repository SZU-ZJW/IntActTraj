...
class Agent:
    ...
    @func_set_timeout(2000)  # 设定forward方法的执行超时时间为200秒
    def forward_with_timeout(self, observation, available_actions, state):
        return self.forward(observation, available_actions, state)
    ...
    def run(
        ...
        while not done:
            ...
            try:
                thought, action, output = self.forward_with_timeout(  # 使用带超时装饰器的forward方法
                    observation, env.get_available_actions(), state
                )
            except func_timeout.exceptions.FunctionTimedOut:
                logger.warning("Forward method timed out. Skipping to the next task.")
                break
